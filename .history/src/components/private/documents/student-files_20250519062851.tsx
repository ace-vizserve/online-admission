...
import type { StudentFiles, StudentDetails } from "@/types";
...
function StudentFiles({ studentID }: ProfileProps) {
  const { data, isPending } = useQuery({
    ...
    queryFn: async () => {
      const [studentDetails, familyDocs] = await Promise.all([
        getStudentDetails({ studentID }),
        getFamilyDocuments(studentID)
      ]);
      return { 
        ...
        familyDocuments: familyDocs.map(doc => ({
          id: 0,
          created_at: new Date().toISOString(),
          documentOwner: doc.owner as "mother" | "father" | "guardian",
...
function FamilyDocuments({label, documents}: { label: string; documents: FamilyDocument[] }) {
  const motherDocuments = documents.filter((doc) => doc.documentOwner === "mother");
  const fatherDocuments = documents.filter((doc) => doc.documentOwner === "father");
  const guardianDocuments = documents.filter((doc) => doc.documentOwner === "guardian");
  ...
  return (
    <div className="space-y-8 py-6 xl:py-0">
      <div className="space-y-2">
        <h1 className="font-bold text-2xl md:text-3xl">{label}</h1>
        <p className="text-sm text-muted-foreground">
          This section includes details about the family student's documents for this current school year.
        </p>
      </div>
