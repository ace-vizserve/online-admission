import { getStudentDetails, getFamilyDocuments, getStudentDocumentsList } from "@/actions/private";
...
function StudentFiles({ studentID }: ProfileProps) {
  const { data, isPending } = useQuery({
    queryKey: ["student-documents-list", studentID],
    queryFn: async () => await getStudentDocumentsList(studentID),
    enabled: !!studentID,
  });
    ...
    <Tabs
      ...
      {tabs.map((tab) => (
        <TabsContent className="w-full px-2" key={tab.value} value={tab.value}>
          <InfoBox documents={data} label={tab.name} value={tab.value} />
        </TabsContent>
      ))}
    </Tabs>
  );
...
function InfoBox({ label, value, documents }: { label: string; value: string; documents: Document[] }) {
  const studentID = documents[0]?.studentID;
  const { data: familyDocuments = [] } = useQuery({
    queryKey: ["family-documents", studentID],
    queryFn: () => getFamilyDocuments(studentID),
    enabled: !!studentID,
  });

  switch (value) {
    case "student-documents":
      return <StudentDocuments label={label} documents={documents} />;
...
function FamilyDocuments({ label, documents }: { label: string; documents: FamilyDocument[] }) {
  const motherDocuments = documents.filter((doc) => 
    doc.owner === "mother" && 
    (doc.documentType === "Passport" || doc.documentType === "Pass")
  );

  const fatherDocuments = documents.filter((doc) => 
    doc.owner === "father" && 
    (doc.documentType === "Passport" || doc.documentType === "Pass")
  );

  const guardianDocuments = documents.filter((doc) => 
    doc.owner === "guardian" && 
    (doc.documentType === "Passport" || doc.documentType === "Pass")
  );
  ...
  return (
    <div className="space-y-8 py-6 xl:py-0">
      <div className="space-y-2">
        <h1 className="font-bold text-2xl md:text-3xl">{label}</h1>
        <p className="text-sm text-muted-foreground">
          This section includes details about the family members' documents.
        </p>
      </div>
        ...
        <>
          ...
          <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-3 gap-y-4">
            {motherDocuments.map((doc) => (
              <DocumentCard key={doc.owner + doc.documentType + doc.fileUrl} doc={doc} />
            ))}
          </div>
        </>
      )}
            ...
            <PopoverContent className="w-72">
              <div className="grid gap-4">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground">See the details of the {doc.documentType}.</p>
                </div>
                <div className="grid gap-2">
                  {doc.documentType === "Pass" && (
                    <>
                      {/* passExpirationDate does not exist on FamilyDocument, so this field is removed */}
                    </>
                  )}
                  {doc.documentType === "Passport" && (
                    <>
                      {/* passportNumber does not exist on FamilyDocument, so this field is removed */}
                    </>
                  )}
                </div>
              </div>
