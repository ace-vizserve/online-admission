...
export const columns: ColumnDef<TStudent>[] = [
  {
    ...
    cell: ({ row }) => <div className="capitalize text-xs pl-4">{row.getValue("documentType")}</div>,
  },
  {
    accessorKey: "status",
    header: ({ column }) => {
    ...
    cell: ({ row }) => <div className="text-xs tabular-nums pl-1">{row.getValue("status")} years old</div>,
  },
  {
      ...
      return (
        <DropdownMenu>
          ...
          <DropdownMenuContent className="mt-2">
            <Link to={`/admission/document-file/${student.studentID}`}>
