...
/**
 * Local+remote-merged equivalent of getDraftRows (src/components/private/drafts/draft-ticket.tsx):
 * merges in the server's copy of each flow's drafts so a draft started on another
 * browser/device shows up here too.
 */
export async function getMergedDraftRows(): Promise<DraftRow[]> {
  const [hfse, viz] = await Promise.all([syncDraftsForType("hfse-is"), syncDraftsForType("viz-school")]);
