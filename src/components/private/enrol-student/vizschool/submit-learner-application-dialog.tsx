...
function SubmitLearnerApplicationDialog() {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async () => {
      return await submitVizSchoolEnrollment(formState as VizSchoolEnrolOldStudentFormState, academicYear, schoolFee);
    },
    onSuccess() {
    ...
    try {
      ...
      if (isNaN(Number(contactPersonNumber))) {
        toast.warning("Invalid Contact Person Number!", {
          ...
          action: {
            label: "Edit Info",
            onClick: () => navigate(`/vizschool/enrol-student/${params.id}/student-info?academicYear=${academicYear}`),
          },
          actionButtonStyle: {
      ...
      if (formState.familyInfo == null) {
        toast.info("Review Family Information!", {
          ...
          action: {
            label: "View Info",
            onClick: () => navigate(`/vizschool/enrol-student/${params.id}/family-info?academicYear=${academicYear}`),
          },
          actionButtonStyle: {
      ...
      if (formState.enrollmentInfo == null) {
        toast.warning("Fill up the enrollment information tab!", {
          ...
          action: {
            label: "Edit Info",
            onClick: () =>
              navigate(`/vizschool/enrol-student/${params.id}/enrollment-info?academicYear=${academicYear}`, {
                state: {
                  triggerForm: true,
                },
              }),
      ...
      if (formState.uploadRequirements?.studentUploadRequirements == null) {
        toast.warning("Please upload the required student documents", {
          ...
          action: {
            label: "Edit Info",
            onClick: () => navigate(`/vizschool/enrol-student/${params.id}/documents?academicYear=${academicYear}`),
          },
          actionButtonStyle: {
            backgroundColor: "#DC7609",
          },
...
export default SubmitLearnerApplicationDialog;
