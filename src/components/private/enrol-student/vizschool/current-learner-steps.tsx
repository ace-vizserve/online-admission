...
function OldStudentSteps() {
  const { formState } = useEnrolCurrentLearnerContext();
    ...
    <nav className="w-full bg-white mb-12 md:mb-0">
      <ol className="flex flex-col lg:flex-row max-w-screen mx-auto">
        {STEPS.map((step, index) => (
          <NavLink
            key={step.path}
            to={`/vizschool/enrol-student/${params.id}/${step.path}?academicYear=${academicYear}`}
                ...
                <div className="absolute bottom-0 left-0 w-full h-1 px-2">
                  <div
                    className={cn(
                      ...
                      isActive ? (step.invalid ? "bg-destructive" : "bg-secondary") : "bg-slate-100"
                    )}
                  />
                </div>
              </div>
