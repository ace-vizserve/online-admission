...
import { ArrowUpDown, ClipboardList, MoreHorizontal, RefreshCcw, Search, UserPlus } from "lucide-react";
    ...
    cell: ({ row }) => (
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[200px] max-w-[85vw] p-0 rounded-xl border-slate-200 shadow-xl">
          <Link to={`/admission/enrolments/${row.original.studentNumber}`}>
            <DropdownMenuItem className="flex items-center gap-3 px-3 py-3 md:py-2.5 cursor-pointer rounded-lg group focus:bg-primary text-primary focus:text-white">
              <div className="flex items-center justify-center size-9 md:size-8 rounded-lg bg-indigo-50 group-focus:bg-white/20">
                <ClipboardList className="size-5 md:size-4 text-primary group-focus:text-white" />
              </div>
              <div className="flex flex-col">
                <span className="text-sm font-bold">View Enrolments</span>
                <span className="text-[10px] md:text-[12px] opacity-80 font-medium">Full student records</span>
              </div>
            </DropdownMenuItem>
          </Link>
        </DropdownMenuContent>
...
function Enrol() {
  ...
  if (isPending) {
    return (
      <div className="h-96 w-full flex flex-col gap-4 items-center justify-center my-10">
        <Tailspin size="40" stroke="5" speed="0.9" color="#4F46E5" />
        <p className="text-sm font-bold text-muted-foreground animate-pulse">Loading student data...</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full py-8">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
        <div>
          <h1 className="font-extrabold text-2xl md:text-3xl text-primary tracking-tight">Student Enrolments</h1>
          <p className="text-sm font-medium text-muted-foreground">Search and view specific enrolment histories</p>
        </div>
