...
import { AlertTriangleIcon, ArrowRight, Calendar as CalendarIcon, CheckCircle, Info } from "lucide-react";
...
function FatherInformation() {
  ...
  const { formState, setFormState, setCompletedTabs, setCurrentTab, setActiveTab, completedTabs } =
    useOpenHouseContext();
  ...
  async function hasFatherInfoToggle(checked: boolean) {
    if (checked) {
      ...
      form.reset({ ...(formState.familyInfo?.fatherInfo ?? {}), noFatherInfo: false });
      setFormState({
        ...formState,
        familyInfo: {
          ...formState.familyInfo!,
