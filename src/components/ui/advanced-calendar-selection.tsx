...
const AdvancedCalendarSelection = ({ disablePastDates, date = new Date(), setDate }: CalendarProps) => {
  const today = new Date(date);

  const [month, setMonth] = useState(today);
  const [isYearView, setIsYearView] = useState(false);

  const startDate = new Date(1900, 6);
  const endDate = new Date(2100, 6);
...
export default AdvancedCalendarSelection;
