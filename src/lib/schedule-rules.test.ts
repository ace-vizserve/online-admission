...
describe("scheduleOptionsForLevel", () => {
  ...
  it("keeps Morning/Afternoon for Standard and Enrichment at the unrestricted levels", () => {
    ...
    expect(scheduleOptionsForLevel("Primary Four", STANDARD)).toEqual(["Morning", "Afternoon"]);
  ...
  it("restricts exactly the two levels that exceeded morning capacity, on exactly one class type", () => {
    expect(AFTERNOON_ONLY_CLASS_LEVEL).toEqual(["Primary Three", "Primary Six"]);
