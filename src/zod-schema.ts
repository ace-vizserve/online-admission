...
export const studentDetailsSchema = z.object({
  studentPhoto: z.string().min(1, {
    message: "Please upload a student photo to continue.",
  }),
...
export const fatherInformationSchema = z.object({
...
export const siblingInformationSchema = z.object({
  siblings: z.array(
    z.object({
...
export const enrollmentInformationSchema = z.object({
  ...
  discount: z
    .string()
    .refine((val) => !val || val === "AY250H01EN", {
      message: "Enter the correct discount code 'AY250H01EN'.",
    })
    .optional(),
...
export const studentUploadRequirementsSchema = z.object({
  ...
  transcriptOfRecords: z.string().min(1, "Transcript of records is required."),
  form12: z.string().min(1, "Form 12 is required."),
  medicalExam: z.string().min(1, "Medical exam result is required."),
  passport: z.string().min(1, "Student's passport copy is required."),
  passportNumber: z.string().min(1, "Passport number is required."),
  passportExpiryDate: z.coerce.date({
    errorMap: () => ({ message: "Enter a valid passport expiry date." }),
  }),
  pass: z.string().min(1, "Student's pass is required."),
...
export const parentGuardianUploadRequirementsSchema = z.object({
  passport: z.string().min(1, "Parent/Guardian's passport copy is required."),
  passportNumber: z.string().min(1, "Passport number is required."),
  passportExpiryDate: z.coerce.date({
    errorMap: () => ({ message: "Enter a valid passport expiry date." }),
  }),
  pass: z.string().min(1, "Parent/Guardian's pass is required."),
  passType: z.string().min(1, "Pass type is required."),
  passExpiryDate: z.coerce.date({
    errorMap: () => ({ message: "Enter a valid pass expiry date." }),
  }),
